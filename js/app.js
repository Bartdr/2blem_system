...
// ===================================================
// TURNOS EN TIEMPO REAL DESDE FIRESTORE
// ===================================================
function escucharTurnos() {
  const lista = document.getElementById('turnos-list');
  const contEspera = document.getElementById('t-espera');
  const contActual = document.getElementById('t-actual');
  if (!lista) return;

  // Escucha en tiempo real — solo citas de hoy con estado pendiente o en-silla
  ...
  onSnapshot(q, (snapshot) => {
    ...
    // Filtrar solo pendientes y en-silla
    const activas = citas.filter(c => c.estado === "pendiente" || c.estado === "en-silla");
    const enSilla = activas.filter(c => c.estado === "en-silla");
    const enEspera = activas.filter(c => c.estado === "pendiente");

    // Actualizar contadores
    if (contEspera) contEspera.textContent = enEspera.length;
    if (contActual) contActual.textContent = enSilla.length;
    ...
    lista.innerHTML = activas.map((cita, index) => {
      const esEnSilla = cita.estado === "en-silla";
      const minutos   = enEspera.indexOf(cita) * 15; // ~15 min por turno

      let badge = '';
      if (esEnSilla) {
        badge = `<div class="turno-badge green">En silla ✂️</div>`;
      } else if (index === enSilla.length) {
        badge = `<div class="turno-badge yellow">Próximo</div>`;
      } else {
        badge = `<div class="turno-badge">~${minutos} min</div>`;
      }

      return `
        <div class="turno-item ${esEnSilla ? 'current' : 'waiting'}">
          <div class="turno-num">#${index + 1}</div>
          <div class="turno-info">
            <strong>${cita.nombre}</strong>
            <span>${cita.servicio}</span>
            <span style="font-size:0.75rem;color:#555;">${cita.hora}</span>
          </div>
          ${badge}
        </div>
      `;
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  loadPhones();
  escucharTurnos(); // ✅ inicia escucha en tiempo real
});
